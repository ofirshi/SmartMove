...
def provideServerServiceKey(serverService):
    ...
    if 'port' in serverService:  # key for TCP or UDP or SCTP
        ...
        if 'icmp-code' in serverService and serverService['icmp-code'] is not 'null':
...
def processServices(client, userServices, userServiceType):
    ...
    for serverService in res_get_services.data:
        ...
        if 'port' in serverService and ('protocol' not in serverService or serverService['protocol'] is 'null'):
    ...
    for userService in userServices:
        ...
        if 'Port' in userService:
            ...
            if 'Code' in userService and userService['Code'] is not 'null':
...
# ---
# returns: nothing
def addAccessRules(client, userRules, userLayerName, skipCleanUpRule, mergedNetworkObjectsMap, mergedServiceObjectsMap, mergedTimesGroupsNamesMap):
    ...
    if userPackage is not None:
        ...
        if userPackage['SubPolicies'] is not None:
            for userSubLayer in userPackage['SubPolicies']:
                ...
                printStatus(None, "")
                publishCounter = publishUpdate(publishCounter, True)
                addAccessRules(client, userSubLayer['Rules'], userSubLayer['Name'], False, mergedNetworkObjectsMap, mergedServiceObjectsMap, mergedTimesGroupsNamesMap)
        if userPackage['ParentLayer'] is not None:
            addAccessRules(client, userPackage['ParentLayer']['Rules'], "parent", True, mergedNetworkObjectsMap, mergedServiceObjectsMap, mergedTimesGroupsNamesMap)
    return addedPackage
    ...
    with APIClient(client_args) as client:
        ...
        if client.check_fingerprint() is False:
            ...
            if login_res.success is False:
                ...
                addedPackage = processPackage(client, userPackage, mergedNetworkObjectsMap, mergedServicesObjectsMap, mergedTimesGroupsMap)
